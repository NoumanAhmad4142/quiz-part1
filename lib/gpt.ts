...
export class GptService {
  ...
  constructor() {
    this.openAi = new OpenAI({
      ...
      apiKey: process.env["NEXT_OPENAI_API_KEY"], // This is the default and can be omitted
    });
  }
}
    ...
    try {
      ...
      for (let index = 0; index < output.length; index++) {
        ...
        if (output[index].answer) {
          const optionKeys = Object.keys(output[index]).filter((key) =>
          ...
          if (!optionKeys.includes(output[index].answer)) {
            ...
            if (emptyOptionKey) {
              output[index][emptyOptionKey] = output[index].answer;
            } else {
